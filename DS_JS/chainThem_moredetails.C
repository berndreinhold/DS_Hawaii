...
int main(){
  ...
  //string dir="/pnfs/darkside/scratch/users/reinhol1/Internal/FastSim/";
  string dir="/pnfs/darkside/scratch/users/reinhol1/Internal/NullField_Uniform/";
  //string dir="/pnfs/darkside/scratch/users/reinhol1/CalibData/CopperRings_NullField_center_plus26mm/";
  //string dir="/pnfs/darkside/scratch/users/reinhol1/CalibData/CopperRings_200V_ExtLarScint0_center_plus26mm/";
  ...
  //string label="CopperRings_200V_ExtLarScint0";
  //string label="CopperRings_NullField";
  //string zposition="center_plus26mm_";
  //string isotope="Co57"; //macro name, Cs137, Ba133, Th232
  
  
  //string label="FastSim";
  string label="NullField_Uniform";
  string zposition="";
  //if (zposition.length()>0) zposition = "_" + zposition + "_";
  string isotope="Kr83m"; //macro name, Cs137, Ba133, Th232
  
  cout << "label: " << label << endl;
  cout << "zposition: " << zposition << endl;
  cout << "isotope: " << isotope << endl;
  string outfile=Form("%s_%s_full%s.root",isotope.c_str(), label.c_str(),zposition.c_str());
  ...
  */
  ...
  TCut tC="cl_npe>1";
  ch->Draw("ev:nclus:npe:cl_x",tC,"goff"); //one common n-tp
  cout << "selected entries: " << ch->GetSelectedRows() << endl;
  std::vector<float> v1;
  std::vector<float> v2;
  std::vector<float> v3;
  //the cl_x is only used out of the array structure, to ensure that all queries have the same structure. Without it, the above would be the only query without a cl_* variable.
